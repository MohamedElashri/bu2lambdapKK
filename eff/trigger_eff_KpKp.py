...
with tqdm(total=total_trees, desc="Processing trees") as pbar:
    for track in ["LL", "DD"]:
        ...
        for year in ["16", "17", "18"]:
            ...
            debug_print(
                f"Track: {track}, Year: {year}, Combined before trigger events: {before_trigger_evts}"
            )
...
print_table_both_formats(
    caption="Trigger efficiencies for $B^+ \\to \\Lambda^0_{\\text{LL}} \\bar{p} K^+ K^+$ selection (\\%)",
    label="tab:trigger_eff_LL_pbar",
...
print_table_both_formats(
    caption="Trigger efficiencies for $B^+ \\to \\Lambda^0_{\\text{DD}} \\bar{p} K^+ K^+$ selection (\\%)",
    label="tab:trigger_eff_DD_pbar",
    headers=["2016", "2017", "2018"],
    row_names=triggers,
    data=efficiency,
    track_type="DD",
...
if args.l0_breakdown:
    ...
    print_table_both_formats(
        caption="$L0$ contributions for $B^+ \\to \\Lambda^0_{\\text{LL}} \\bar{p} K^+ K^+$ selection (\\%)",
        label="tab:l0_tis_breakdown_LL_pbar",
        headers=["2016", "2017", "2018"],
    ...
    print_table_both_formats(
        caption="$L0$ contributions for $B^+ \\to \\Lambda^0_{\\text{DD}} \\bar{p} K^+ K^+$ selection (\\%)",
        label="tab:l0_tis_breakdown_DD_pbar",
        headers=["2016", "2017", "2018"],
        row_names=l0_tis_with_total,
        data=l0_tis_breakdown_with_total,
        track_type="DD",
