...
def find_tree_path(fpath: str, tracks) -> str:
    ...
    Updated to handle both directory structures.
    """
    ...
    for tr in tracks:                     # honour order in config.yml
        ...
        for k in keys:
            k_name = k.rstrip(";1")
            if k_name.endswith(f"_{tr}") and isinstance(rf[k], uproot.TTree):
                return f"{fpath}:{k_name}"

        # 2) case: directory → DecayTree
        for k in keys:
            k_name = k.rstrip(";1")
            if k_name.endswith(f"_{tr}") and isinstance(rf[k], uproot.ReadOnlyDirectory):
                # Check if DecayTree exists in this directory
                directory = rf[k_name]
                if "DecayTree;1" in directory.keys():
                    candidate = f"{fpath}:{k_name}/DecayTree"
                    try:
                        uproot.open(candidate)
                        return candidate
                    except KeyError:
                        continue
                # Otherwise just return the directory itself
                return f"{fpath}:{k_name}"
...
def branch_set(tree_path: str) -> set:
    ...
    try:
        ...
        print(f"Error getting branches from {tree_path}: {e}")
        return set()
...
for cls, (base, patt) in MAP.items():
    try:
        ...
        if fpath:
            ...
            print(f"No file found for {cls} with pattern {patt} in {base}")
    except Exception as e:
...
if branch_sets:
    common = set.intersection(*branch_sets.values()) if branch_sets else set()
    ...
    print("No branch sets found to compare")