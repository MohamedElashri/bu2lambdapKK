...
class BackgroundModelComparison:
    ...
    def create_comparison_plot(
        ...
        # ARGUS fit results box (top right, compact)
        pave1: ROOT.TPaveText = ROOT.TPaveText(0.58, 0.62, 0.93, 0.88, "NDC")
        pave1.SetFillColor(ROOT.kWhite)
        pave1.SetFillStyle(1001)
        pave1.SetBorderSize(1)
        pave1.SetTextFont(132)
        pave1.SetTextSize(0.025)
        pave1.SetTextAlign(12)  # Left-aligned
        pave1.SetMargin(0.05)
        pave1.AddText("#font[62]{ARGUS bkg}")
        # Combine yields in fewer lines
        n_jpsi: float = yields_argus["jpsi"].getVal()
        n_etac: float = yields_argus["etac"].getVal()
        n_chic0: float = yields_argus["chic0"].getVal()
        n_chic1: float = yields_argus["chic1"].getVal()
        n_etac2s: float = yields_argus["etac_2s"].getVal()
        n_bkg: float = yields_argus["background"].getVal()
        pave1.AddText(f"N(J/#psi)={n_jpsi:.0f}, N(#eta_{{c}})={n_etac:.0f}")
        pave1.AddText(f"N(#chi_{{c0}})={n_chic0:.0f}, N(#chi_{{c1}})={n_chic1:.0f}")
        pave1.AddText(f"N(#eta_{{c}}(2S))={n_etac2s:.0f}, N(bkg)={n_bkg:.0f}")
        pave1.Draw()
        ROOT.SetOwnership(pave1, False)
        self._keep_alive.append(pave1)
        # Template fit plot
        pad2.cd()
        ...
        # Template fit results box (top right, compact)
        pave2: ROOT.TPaveText = ROOT.TPaveText(0.58, 0.62, 0.93, 0.88, "NDC")
        pave2.SetFillColor(ROOT.kWhite)
        pave2.SetFillStyle(1001)
        pave2.SetBorderSize(1)
        pave2.SetTextFont(132)
        pave2.SetTextSize(0.025)
        pave2.SetTextAlign(12)  # Left-aligned
        pave2.SetMargin(0.05)
        pave2.AddText("#font[62]{Template bkg}")
        # Combine yields in fewer lines
        n_jpsi: float = yields_template["jpsi"].getVal()
        n_etac: float = yields_template["etac"].getVal()
        n_chic0: float = yields_template["chic0"].getVal()
        n_chic1: float = yields_template["chic1"].getVal()
        n_etac2s: float = yields_template["etac_2s"].getVal()
        n_bkg: float = yields_template["background"].getVal()
        pave2.AddText(f"N(J/#psi)={n_jpsi:.0f}, N(#eta_{{c}})={n_etac:.0f}")
        pave2.AddText(f"N(#chi_{{c0}})={n_chic0:.0f}, N(#chi_{{c1}})={n_chic1:.0f}")
        pave2.AddText(f"N(#eta_{{c}}(2S))={n_etac2s:.0f}, N(bkg)={n_bkg:.0f}")
        pave2.Draw()
        ROOT.SetOwnership(pave2, False)
        self._keep_alive.append(pave2)
        canvas.Update()
