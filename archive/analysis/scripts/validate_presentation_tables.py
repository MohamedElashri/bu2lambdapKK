...
class TableValidator:
    ...
    def validate_yields(self):
        ...
        for state in states:
            for year in years:
                data = source[(source["state"] == state) & (source["year"] == year)]
                if len(data) == 0:
        ...
        for _, row in source.iterrows():
            ...
            if e > y:
                self.warnings.append(
                    f"{row['state']}_{row['year']}: " f"Error ({e:.1f}) > Yield ({y:.1f})"
                )

            if e < 0:
                self.errors.append(f"{row['state']}_{row['year']}: Negative error ({e:.1f})")
    ...
    def validate_br_ratios(self):
        ...
        for num, den in expected_ratios:
            ratio_data = source[(source["numerator"] == num) & (source["denominator"] == den)]
...
def main():
    ...
    validator = TableValidator(tables_dir=str(tables_dir), presentation_dir=str(presentation_dir))
