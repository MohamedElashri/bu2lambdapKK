...
from .exceptions import ConfigurationError, DataLoadError
...
class TOMLConfig:
    ...
    def _load_toml(self, filename: str) -> dict:
        ...
        try:
            ...
            raise ConfigurationError(f"Error parsing TOML file {config_path}: {e}")
...
class FourMomentumCalculator:
    ...
    @staticmethod
    def calculate_invariant_mass_lpkm(
        events: ak.Array,
        lambda_branches: dict[str, str],
        bachelor_p_branches: dict[str, str],
        kminus_branches: dict[str, str],
    ) -> ak.Array:
        """
...
class DataManager:
    ...
    def load_tree(
        ...
        try:
            ...
            raise DataLoadError(f"Error reading ROOT file {filepath}: {e}")
    ...
    def compute_derived_branches(self, events: ak.Array) -> ak.Array:
        ...
        if "L0_MM" in events.fields and "Bu_MM" in events.fields:
            events = ak.with_field(
                events, events["Bu_MM"] - events["L0_MM"] + lambda_mass_pdg, "Bu_MM_corrected"
            )
        ...
        if all(
            ...
            Delta_X_ERR = np.sqrt(
                events["Bu_ENDVERTEX_XERR"] ** 2 + events["L0_ENDVERTEX_XERR"] ** 2
            )
            Delta_Y_ERR = np.sqrt(
                events["Bu_ENDVERTEX_YERR"] ** 2 + events["L0_ENDVERTEX_YERR"] ** 2
            )
            Delta_Z_ERR = np.sqrt(
                events["Bu_ENDVERTEX_ZERR"] ** 2 + events["L0_ENDVERTEX_ZERR"] ** 2
            )
        ...
        if all(
            ...
            M_Lp_h1 = self.four_momentum_calc.calculate_invariant_mass_lpkm(
                events, lambda_4mom_branches, bachelor_p_4mom_branches, h1_4mom_branches
            )

            M_Lp_h2 = self.four_momentum_calc.calculate_invariant_mass_lpkm(
                events, lambda_4mom_branches, bachelor_p_4mom_branches, h2_4mom_branches
            )
    ...
    def load_all_data_combined_magnets(
        self, particle_type: str, track_types: list[str] = ["LL", "DD"]
    ) -> dict[str, ak.Array]:
        """
        ...
        for year in self.config.paths["data"]["years"]:
            ...
            for track_type in track_types:
                events = self.load_and_process(
                    particle_type, year, track_type, apply_derived_branches=True, apply_trigger=True
                )
                if events is not None:
