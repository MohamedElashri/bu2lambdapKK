...
class CacheManager:
    ...
    def load(
        ...
        if validate and metadata_path.exists():
            try:
                ...
                if metadata.version != self.CACHE_VERSION:
                    print(f"  ⚠️  Cache version mismatch: {metadata.version} != {self.CACHE_VERSION}")
                    return None
        ...
        try:
            ...
            print(f"  ✓ Loaded from cache: {name} ({data_path.stat().st_size / 1024 / 1024:.1f} MB)")
            return data
    ...
    def invalidate(self, name: str, dependencies: Optional[Dict[str, str]] = None) -> None:
        """
