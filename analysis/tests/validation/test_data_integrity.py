...
class TestDataShapes:
    ...
    def test_physics_data_structure(self, mock_data_array: np.ndarray) -> None:
        ...
        assert isinstance(mock_data_array, np.ndarray), \
            "Data should be numpy array"
        assert mock_data_array.dtype.names is not None, \
            "Data should have named fields"
    ...
    def test_physics_data_field_consistency(self, mock_data_array: np.ndarray) -> None:
        ...
        for field in mock_data_array.dtype.names:
            ...
            assert len(field_data) == n_events, \
...
class TestNaNInfValues:
    ...
    def test_no_nan_in_masses(self, mock_data_array: np.ndarray) -> None:
        ...
        for field in mass_fields:
            ...
            assert not np.any(np.isnan(data)), \
                f"Field {field} contains NaN values"
    ...
    def test_no_inf_in_masses(self, mock_data_array: np.ndarray) -> None:
        ...
        for field in mass_fields:
            ...
            assert not np.any(np.isinf(data)), \
                f"Field {field} contains inf values"
    ...
    def test_efficiency_no_nan(self) -> None:
        ...
        assert not np.isnan(jpsi_2016['eff']), \
            "Efficiency should not be NaN"
        assert not np.isnan(jpsi_2016['err']), \
            "Efficiency error should not be NaN"
    ...
    def test_efficiency_no_inf(self) -> None:
        ...
        assert not np.isinf(jpsi_2016['eff']), \
            "Efficiency should not be inf"
        assert not np.isinf(jpsi_2016['err']), \
            "Efficiency error should not be inf"
    ...
    def test_yield_no_nan(self) -> None:
        ...
        assert not np.isnan(jpsi_yield[0]), \
            "Yield should not be NaN"
        assert not np.isnan(jpsi_yield[1]), \
            "Yield error should not be NaN"
...
class TestDataTypeConsistency:
    ...
    def test_efficiency_is_numeric(self) -> None:
        ...
        assert isinstance(jpsi_2016['eff'], (int, float, np.number)), \
            "Efficiency should be numeric"
        assert isinstance(jpsi_2016['err'], (int, float, np.number)), \
    ...
    def test_yield_is_numeric(self) -> None:
        ...
        assert isinstance(jpsi_yield[0], (int, float, np.number)), \
            "Yield should be numeric"
        assert isinstance(jpsi_yield[1], (int, float, np.number)), \
            "Yield error should be numeric"
...
class TestEmptyData:
    ...
    def test_missing_field_detected(self, mock_data_array: np.ndarray) -> None:
        ...
        assert 'nonexistent_field' not in mock_data_array.dtype.names, \
...
class TestDataRanges:
    ...
    def test_masses_positive(self, mock_data_array: np.ndarray) -> None:
        ...
        for field in mass_fields:
            ...
            assert np.all(data > 0), \
                f"Field {field} contains non-positive values"
    ...
    def test_efficiency_in_range(self) -> None:
        ...
        assert 0 <= jpsi_2016['eff'] <= 1, \
            f"Efficiency {jpsi_2016['eff']} out of range [0,1]"
    ...
    def test_efficiency_error_positive(self) -> None:
        ...
        assert jpsi_2016['err'] >= 0, \
            f"Efficiency error {jpsi_2016['err']} should be non-negative"
    ...
    def test_yield_non_negative(self) -> None:
        ...
        assert jpsi_yield[0] >= 0, \
            f"Yield {jpsi_yield[0]} should be non-negative"
    ...
    def test_yield_error_positive(self) -> None:
        ...
        assert jpsi_yield[1] >= 0, \
            f"Yield error {jpsi_yield[1]} should be non-negative"
    ...
    def test_pt_positive(self, mock_data_array: np.ndarray) -> None:
        ...
        for field in pt_fields:
            data = mock_data_array[field]
            assert np.all(data > 0), \
                f"Field {field} contains non-positive PT values"
...
class TestDataConsistency:
    ...
    def test_error_smaller_than_value(self) -> None:
        ...
        if jpsi_yield[0] > 0:
            ...
            assert rel_error < 1.0, \
                f"Relative error {rel_error:.2%} seems unreasonably large"
    ...
    def test_efficiency_error_reasonable(self) -> None:
        ...
        assert jpsi_2016['err'] <= jpsi_2016['eff'], \
            "Efficiency error should not exceed efficiency value"
        assert jpsi_2016['err'] < 0.5, \
            "Efficiency error seems unreasonably large"
    ...
    def test_mass_correlations_sensible(self, mock_data_array: np.ndarray) -> None:
        ...
        if bu_field and lambda_field:
            ...
            assert fraction > 0.9, \
                f"Only {fraction:.1%} of events have Bu_M > L0_MM"
...
class TestDataCompleteness:
    ...
    def test_all_expected_fields_present(self, mock_data_array: np.ndarray) -> None:
        ...
        assert len(mock_data_array.dtype.names) > 0, \
            "Data should have at least one field"
        
        # Common fields should be present
        expected_fields = ['Bu_PT', 'Bu_M', 'L0_MM']
        for field in expected_fields:
            assert field in mock_data_array.dtype.names, \
    ...
    def test_sufficient_statistics(self) -> None:
        ...
        assert n_events >= 100, \
            "Should have sufficient events for analysis"
    ...
    def test_all_years_have_data(self) -> None:
        ...
        for year in years:
            ...
            for state in states:
                assert state in yield_data[year], \
                    f"Missing state {state} for year {year}"
