...
class SelectionStudy:
    ...
    def _plot_mc_distribution(self, mc_data: ak.Array, var_config: dict, output_name: str):
        """
        Plot MC-only distribution for Phase 1 optimization
        
        Parameters:
        - mc_data: J/ψ MC data
        - var_config: Variable configuration
        - output_name: Output file path (without extension)
        """
        branch = var_config['branch']
        ...
        mc_vals = safe_extract(mc_data, branch)
        ...
        plot_range = var_config.get('plot_range', [np.min(mc_vals), np.max(mc_vals)])
        n_bins = 50
        
        # Plot MC histogram
        ax.hist(mc_vals, bins=n_bins, range=plot_range,
               histtype='stepfilled', alpha=0.3, color='#E41A1C',
               label='J/ψ MC')
        ax.hist(mc_vals, bins=n_bins, range=plot_range,
               histtype='step', linewidth=2, color='#E41A1C')
        ...
        ax.set_xlabel(var_config.get('description', branch))
        ax.set_ylabel('Events')
        ax.set_title(f'{var_config.get("description", branch)} - MC Optimization')
        ax.legend()
        ax.grid(True, alpha=0.3)
        
        # Save
        output_path = self.phase_output_dir / f"{output_name}.pdf"
    ...
    def study_lambda_selection(self):
        """
        Phase 1: Study Lambda selection variables (MC optimization)
        """
        self.logger.info("="*80)
        self.logger.info("Optimizing Lambda Selection (MC)")
        self.logger.info("="*80)
        
        lambda_vars = self.config['lambda_variables']
        output_dir = self.phase_output_dir / "lambda_optimization"
        ...
        for var_name, var_config in lambda_vars.items():
            ...
            # Plot MC distribution only (no data sidebands in Phase 1)
            self._plot_mc_distribution(
                self.jpsi_signal, var_config, 
                f"lambda_optimization/lambda_{var_name}_dist"
            )
            ...
            if 'scan_range' in var_config:
                ...
                self.var_analyzer.plot_efficiency_scan(
                    ...
                    f"lambda_optimization/lambda_{var_name}_effscan",
        ...
        self.study_plotter.plot_combined_efficiency(
            ...
            f"lambda_optimization/combined_lambda_efficiency"
        )
        ...
        if 'lambda_fdchi2' in lambda_vars and 'delta_z' in lambda_vars:
            self.study_plotter.plot_2d_correlation(
                ...
                lambda_vars['delta_z'],
                f"lambda_optimization/2d_lambda_optimization",
    ...
    def study_pid_selection(self):
        """
        Phase 1: Study PID selection variables (MC optimization)
        """
        self.logger.info("="*80)
        self.logger.info("Optimizing PID Selection (MC)")
        self.logger.info("="*80)
        
        pid_vars = self.config['pid_variables']
        output_dir = self.phase_output_dir / "pid_optimization"
        output_dir.mkdir(parents=True, exist_ok=True)
        ...
        for var_name, var_config in pid_vars.items():
            ...
            # Plot MC distribution only
            self._plot_mc_distribution(
                self.jpsi_signal, var_config,
                f"pid_optimization/pid_{var_name}_dist"
            )
            ...
            if 'scan_range' in var_config:
                ...
                self.var_analyzer.plot_efficiency_scan(
                    scan_results, var_config,
                    f"pid_optimization/pid_{var_name}_effscan",
                    label="J/$\\psi$ Signal MC"
                )
        ...
        if pid_vars.get('p_probnnp', {}).get('enabled') and pid_vars.get('h1_probnnk', {}).get('enabled'):
            self.study_plotter.plot_2d_correlation(
                ...
                pid_vars['p_probnnp'],
                pid_vars['h1_probnnk'],
                f"pid_optimization/2d_p_vs_h1_efficiency",
                plot_type="efficiency"
            )
        ...
        if pid_vars.get('h1_probnnk', {}).get('enabled') and pid_vars.get('h2_probnnk', {}).get('enabled'):
            self.study_plotter.plot_2d_correlation(
                ...
                pid_vars['h1_probnnk'],
                pid_vars['h2_probnnk'],
                f"pid_optimization/2d_h1_vs_h2_efficiency",
                plot_type="efficiency"
            )
        ...
        self.study_plotter.plot_combined_efficiency(
            ...
            f"pid_optimization/combined_pid_efficiency"
        )
    ...
    def study_bplus_selection(self):
        """
        Phase 1: Study B+ quality variables (MC optimization)
        """
        self.logger.info("="*80)
        self.logger.info("Optimizing B+ Quality Selection (MC)")
        self.logger.info("="*80)
        
        bplus_vars = self.config['bplus_variables']
        output_dir = self.phase_output_dir / "bplus_optimization"
        output_dir.mkdir(parents=True, exist_ok=True)
        ...
        for var_name, var_config in bplus_vars.items():
            ...
            self._plot_mc_distribution(
                self.jpsi_signal, var_config,
                f"bplus_optimization/bplus_{var_name}_dist"
            )
            ...
            if 'scan_range' in var_config:
                ...
                self.var_analyzer.plot_efficiency_scan(
                    scan_results, var_config,
                    f"bplus_optimization/bplus_{var_name}_effscan",
                    label="J/$\\psi$ Signal MC"
                )
        ...
        self.study_plotter.plot_combined_efficiency(
            ...
            f"bplus_optimization/combined_bplus_efficiency"
        )
        
        self.logger.info("\nB+ optimization complete!\n")
    
    def study_jpsi_region(self):
        """
        Phase 2: J/ψ region analysis on real data (with trigger and cuts)
        """
        self.logger.info("="*80)
        self.logger.info("J/ψ Region Analysis (Real Data)")
        self.logger.info("="*80)
        
        output_dir = self.phase_output_dir / "jpsi_analysis"
        output_dir.mkdir(parents=True, exist_ok=True)
        
        # Apply J/ψ region cut to data
        data_region = self.jpsi_analyzer.apply_jpsi_region(self.real_data)
        ...
        self.logger.info(f"  Real data (full region): {len(data_region):,}")
        self.logger.info(f"  Real data (sidebands): {len(data_sidebands):,}")
        
        # Plot mass spectrum: data only
        self._plot_data_mass_spectrum(data_region, data_sidebands, output_dir)
        
        # Signal window analysis
        signal_window_data = self.jpsi_analyzer.apply_signal_window(data_region)
        
        self.logger.info(f"\nSignal Window Analysis [{self.jpsi_analyzer.jpsi_window[0]}-{self.jpsi_analyzer.jpsi_window[1]} MeV]:")
        self.logger.info(f"  Data in signal window: {len(signal_window_data):,}")
        ...
        signal_estimate = len(signal_window_data) - bkg_estimate
        
        self.logger.info(f"\nBackground Estimation (from sidebands):")
        self.logger.info(f"  Expected background in signal window: {bkg_estimate:.1f}")
        self.logger.info(f"  Estimated signal events: {signal_estimate:.1f}")
        if len(signal_window_data) > 0:
            purity = signal_estimate/len(signal_window_data)*100
            self.logger.info(f"  Signal purity: {purity:.1f}%")
        
        # Plot data distributions for all variables
        self.logger.info("\nCreating data distribution plots...")
        self._create_data_distribution_plots(data_region, data_sidebands)
        
        self.logger.info("\nJ/ψ region analysis complete!\n")
    
    def _plot_data_mass_spectrum(self, data_region: ak.Array, data_sidebands: ak.Array, output_dir: Path):
        """Plot J/ψ mass spectrum for data only (Phase 2)"""
        import matplotlib.pyplot as plt
        import mplhep as hep
        ...
        ax.set_xlabel('$M(pK^-\\bar{\\Lambda})$ [MeV/$c^2$]')
        ax.set_ylabel('Events / bin')
        ax.set_title('J/$\\psi$ Mass Spectrum in Real Data')
        ax.legend()
        ax.grid(True, alpha=0.3)
        ...
        # Save
        output_path = output_dir / "jpsi_mass_data.pdf"
        plt.tight_layout()
        plt.savefig(output_path, dpi=300)
        plt.close()
        
        self.logger.info(f"Saved data mass spectrum: {output_path}")
    
    def _create_data_distribution_plots(self, data_region: ak.Array, data_sidebands: ak.Array):
        """Create distribution plots for all variables in data (Phase 2)"""
        import matplotlib.pyplot as plt
        import mplhep as hep
        
        output_dir = self.phase_output_dir / "variable_distributions"
        output_dir.mkdir(parents=True, exist_ok=True)
        ...
        def safe_extract(data, branch):
            ...
            vals = data[branch]
            try:
                # Check if array is jagged (has variable-length sublists)
                if len(ak.flatten(vals)) != len(vals):
                    # Jagged array - take first element
                    vals = vals[:, 0]
            except:
                # Not jagged, use as-is
                pass
            return ak.to_numpy(vals)
        
        # Collect all variables from config
        all_vars = {}
        for section_name in ['lambda_variables', 'pid_variables', 'bplus_variables']:
            section_vars = self.config.get(section_name, {})
            for var_name, var_config in section_vars.items():
        ...
        self.logger.info(f"Creating {len(all_vars)} data distribution plots...")
        
        for var_name, var_config in all_vars.items():
            branch = var_config['branch']
            
            # Extract data safely
            data_vals = safe_extract(data_region, branch)
            sideband_vals = safe_extract(data_sidebands, branch)
            ...
            plot_range = var_config.get('plot_range', [np.percentile(data_vals, 1), np.percentile(data_vals, 99)])
            n_bins = 50
            
            # Plot data histogram
            ax.hist(data_vals, bins=n_bins, range=plot_range,
                   histtype='stepfilled', alpha=0.3, color='#000000',
                   label='Data (J/ψ region)')
            ax.hist(data_vals, bins=n_bins, range=plot_range,
                   histtype='step', linewidth=2, color='#000000')
            
            # Plot sideband histogram
            ax.hist(sideband_vals, bins=n_bins, range=plot_range,
                   histtype='step', linewidth=2, color='#377EB8',
                   linestyle='--', label='Sidebands (Background)')
            ...
            ax.set_xlabel(var_config.get('description', branch))
            ax.set_ylabel('Events')
            ax.set_title(f'{var_config.get("description", branch)} - Data Distribution')
            ax.legend()
            ax.grid(True, alpha=0.3)
            ...
            # Save
            output_path = output_dir / f"data_{var_name}.pdf"
            plt.tight_layout()
            plt.savefig(output_path, dpi=300)
            plt.close()
        
        self.logger.info(f"Saved {len(all_vars)} data distribution plots to: {output_dir}")
...
Examples:
  ...
  python main.py --phase jpsi
    ...
    if args.phase == 'all':
        ...
        # Generate reports after any phase
