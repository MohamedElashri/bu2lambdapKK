...
def plot_lambda_mass_comparison(mc_events, data_events, year_label, lambda_cuts):
    ...
    if mc_events is not None and len(mc_events) > 0:
        ...
        if len(mc_mass) > 0:
            ...
            ax1.axvline(mass_min, color="red", linestyle="--", linewidth=1.5, label="Cut region")
    ...
    if data_events is not None and len(data_events) > 0:
        ...
        if len(data_mass) > 0:
            ...
            # Add vertical lines for cuts
            ax2.axvline(mass_min, color="red", linestyle="--", linewidth=1.5, label="Cut region")
            ...
            ax2.set_title(
                f"$B^+ \\to \\bar{{\\Lambda}} p K^+ K^-$ - {year_label}",
                fontsize=14,
                fontweight="bold",
            )
...
def main():
    ...
    for year in years:
        ...
        fig = plot_lambda_mass_comparison(
            year_mc_combined, year_data_combined, str(year), lambda_cuts
        )
    ...
    fig = plot_lambda_mass_comparison(combined_mc, combined_data, "Combined", lambda_cuts)
