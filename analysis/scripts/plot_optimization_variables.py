...
def plot_variable_comparison(
    mc_events, data_events, year_label, variable_name, var_props, optimal_cuts_dict
):
    """
    ...
    state_colors = {"jpsi": "red", "etac": "orange", "chic0": "purple", "chic1": "brown"}
    state_labels = {
        "jpsi": r"J/$\psi$",
        "etac": r"$\eta_c$",
        "chic0": r"$\chi_{c0}$",
    ...
    if mc_events is not None and len(mc_events) > 0 and variable_name in mc_events.fields:
        ...
        for i, (state, (cut_value, cut_type)) in enumerate(optimal_cuts_dict.items()):
            ...
            ax1.axvline(
                ...
                linestyle=linestyle,
                linewidth=1.5,
                label=f"{state_labels[state]}: {cut_value:.1f}",
                alpha=0.8,
            )
        ...
        if cut_type == "greater":
            ...
            ax1.axvspan(
                cut_value, var_range_mc[1], alpha=0.15, color="green", label="Accepted region"
            )
        else:  # less
            # Keep events < cut value (shade left side)
            ax1.axvspan(
                var_range_mc[0], cut_value, alpha=0.15, color="green", label="Accepted region"
            )
    ...
    if data_events is not None and len(data_events) > 0 and variable_name in data_events.fields:
        ...
        if cut_type == "greater":
            ax2.axvspan(
                cut_value, var_range_data[1], alpha=0.15, color="green", label="Accepted region"
            )
        else:
            ax2.axvspan(
                var_range_data[0], cut_value, alpha=0.15, color="green", label="Accepted region"
            )
        ...
        title_label = year_label if year_label != "Combined" else "2016-2018"
        ax2.set_title(
            f"$B^+ \\to \\bar{{\\Lambda}} p K^+ K^-$ - {title_label}",
            fontsize=14,
            fontweight="bold",
        )
        ...
        ax2.set_ylabel("Normalized Events", fontsize=12)
        ax2.set_title(
            f"$B^+ \\to \\bar{{\\Lambda}} p K^+ K^-$ - {year_label}", fontsize=14, fontweight="bold"
        )
...
def main():
    ...
    parser.add_argument(
        "--mc-state", type=str, default="Jpsi", help="MC state to use for plots (default: Jpsi)"
    )
    parser.add_argument(
        "--state", type=str, default="jpsi", help="State to get optimal cuts from (default: jpsi)"
    )
    ...
    for year in years:
        ...
        for magnet in ["MD", "MU"]:
            for track_type in track_types:
                ...
                # Load MC
                try:
                    mc_events = data_manager.load_tree(mc_state, year, magnet, track_type)
                ...
                # Load Data
                try:
                    data_events = data_manager.load_tree("data", year, magnet, track_type)
    ...
    for var_name, var_props in OPTIMIZATION_VARIABLES.items():
        ...
        fig = plot_variable_comparison(
            combined_mc, combined_data, "Combined", var_name, var_props, optimal_cuts_for_var
        )
