...
class PresentationTableGenerator:
    ...
    def _generate_combined_cuts_table(self, states, state_names):
        ...
        for var in variables:
            row = {"Variable": example_state[example_state["variable"] == var]["description"].iloc[0]}
            
            for state in states:
                state_data = self.optimal_cuts[
                    (self.optimal_cuts["state"] == state) & 
                    (self.optimal_cuts["variable"] == var)
                ]
        ...
        with open(filepath, 'w') as f:
            ...
            f.write(f"**Figure of Merit:** $FOM = n_{{sig}} / \\sqrt{{n_{{bkg}} + n_{{sig}}}}$\n\n")
    ...
    def generate_yield_tables(self):
        ...
        for state in states:
            ...
            for year in years:
                year_data = self.yields[
                    (self.yields["year"] == year) & 
                    (self.yields["state"] == state)
                ]
        ...
        with open(filepath, 'w') as f:
            ...
            f.write("Yields obtained from simultaneous fits to $M(\\Lambda K^+K^-)$ mass distributions.\n\n")
            f.write(df.to_markdown(index=False))
            f.write("\n\n")
...
def main():
    ...
    # Generate tables
    generator = PresentationTableGenerator(
